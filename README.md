...
| Result            | Result of the hand (folded, lost, mucked, non-sd win, out of hand, won) | won                               | string          |
...
- **The outcome of the hand is registered on every row of its player.** `Result` (folded, lost, mucked, non-sd win, out of hand, won), `Balance` (the amount collected from the pot) and `RevealedShowDownPokerHand` (the combination the platform itself named at showdown) are future events recorded early, exactly like the revealed cards: knowing how the hand ended is what makes the behavior that led there analyzable.
...
| Result      | Result of the hand for the player of the row (folded, lost, mucked, non-sd win, out of hand, won), on every row of the player | won |

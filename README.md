...
| `won without showdown` | Everyone else folded: the pot was collected without showing a card. |
...
| `mucked at showdown` | The player reached showdown and threw the cards away unshown — a loss like `lost at showdown`, kept apart because the table never saw the cards. A winning hand is always revealed by the platform, so a mucked one never won; the file itself reveals the mucked cards, kept in `ShowDown`. |
...
- **The outcome of the hand is registered on every row of its player.** `Result` (folded, won without showdown, won at showdown, lost at showdown, mucked at showdown), `Balance` (the amount collected from the pot) and `RevealedShowDownPokerHand` (the combination the platform itself named at showdown) are future events recorded early, exactly like the revealed cards: knowing how the hand ended is what makes the behavior that led there analyzable.

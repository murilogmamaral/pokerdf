...
|  2 | USD Hold'em No Limit |           3 | $1.84+$0.16 | 3026510091 |         1 | 219372022626 | 2020-10-14 14:33:59 | 2020-10-14 10:33:59 | BRT | I       | None   | [10.0, 20.0] | garciamurilo | ['6h', 'Ks'] |         3 | VillainB     |      3 | None         | big blind   |            20 |     500 | nan    | ['checks', '']       | ['checks', '']    | ['bets', '20']             | ['', '']         | False       | False          | False       | False       | False        | ['4d', 'Qs', 'Qd']   | ['4d', 'Qs', 'Qd', '3s'] | []           | [None, None] | None              | won without showdown |        40 | 20               | nan       | 40          | 0    | [40.0]       |          -1 | None    |
...
| RevealedCards     | Cards the player revealed, at showdown or voluntarily (second is null on single-card shows) | [Ah, Ac]           | list[string]    |
| CardCombination   | Card combination held by the player                          | three of a kind, Aces             | string          |
| Result            | Outcome of the hand for the player, in five self-describing values (see below) | won at showdown  | string          |
...
#### The five values of `Result`

Every player dealt into a hand leaves it in exactly one of five ways, and the value spells out both the outcome and how far the hand went:

| Value | The hand it describes |
|-------|-----------------------|
| `folded` | The player gave the hand up. The street where it happened is already in the action columns. |
| `won without showdown` | Everyone else folded: no showdown took place. The winner can still reveal cards voluntarily — they land in `RevealedCards`, and the category does not change. |
| `won at showdown` | The player showed the cards and collected a pot. |
| `lost at showdown` | The player showed the cards and collected nothing. |
| `mucked at showdown` | The player reached showdown and threw the cards away unshown — a loss like `lost at showdown`, kept apart because the table never saw the cards. A winning hand is always revealed by the platform, so a mucked one never won; the file itself reveals the mucked cards, kept in `RevealedCards`. |

The values follow two conventions, so the common questions are one string predicate away instead of a list kept in the head: every win starts with `won`, whether or not cards were needed, and every showdown ends with `at showdown`, whatever the outcome. `Result.str.startswith("won")` selects every pot collected; `Result.str.endswith("at showdown")` selects every showdown reached.
...
- **The known holdings are inferred on every row, at the moment of the row.** `OwnerC1..C2` and `OwnerCombination` are hand context like the board: they fill every row of the hand, so any behavior can be analyzed against the owner's holding without joins. `RevealedC1..C2` and `RevealedCombination` register, on every row of a player that revealed cards — at showdown or voluntarily, the owner included — what the reveal at the end proved the player was holding: a future event recorded early, which is what makes the behavior that led to it analyzable. The combinations name the best hand each holding makes with the board visible at that moment, each with an integer score from 1 (High Card) to 10 (Royal Flush), ready for aggregation. The evaluator was validated against the platform's own showdown labels: 100% agreement over 70k+ real showdowns.  
...
Everything that makes the data worth analyzing is preserved: the order of the actions, the amounts, the pot, the stacks, the board, the positions — and the cards. Your own hole cards (`OwnerC1`, `OwnerC2`), the cards the players revealed (`RevealedC1`, `RevealedC2`) and the combinations derived from them are kept in every mode, since the decisions in the dataset can only be studied against the holdings they were made with, and cards shown at the table describe the game, not a person.
...
| RevealedC1..C2 | Cards the player of the row revealed — at showdown, or voluntarily after winning without one — registered on every row of that player in the hand | 7h, Td |
| RevealedCombination | Best combination the revealed cards make with the visible board (null when only one card was shown) | One Pair |
| RevealedCombinationScore | The same combination as an integer, from 1 to 10                                 | 2           |
| RevealedPokerHand | Combination the platform itself named at showdown, when the player showed             | a pair of Aces |
| Result      | Outcome of the hand for the player of the row (folded, won without showdown, won at showdown, lost at showdown, mucked at showdown), on every row of the player | won at showdown |
